...
module.exports.initialize = function(config) {
  ...
  app.use('/catimages', express.static(app.locals.config.get("application").repository + '/catimages'));
  //app.use('/', express.static(app.locals.config.get("application").repository + '/'));
  ...
  app.all('/maacs/*', requireAuthentication);

  app.use("/wiki", wikiStatic.configure());

  app.use(require("../routes/maacs"))
